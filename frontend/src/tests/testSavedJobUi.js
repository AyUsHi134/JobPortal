...
import { getSaveButtonState, resolveSavedStateAfterSave } from "../utils/savedJobUi.js";
...
// ---------------------------------------------------------------------------
console.log("\n[4] getSaveButtonState — an already-saved job is a disabled, terminal state (no unsave endpoint exists)");
{
  const state = getSaveButtonState({ isAuthenticated: true, isSaved: true, isSaving: false });
  check("label reads 'Saved'", state.label === "Saved");
  check("disabled — clicking again must not re-POST or attempt a nonexistent unsave call", state.disabled === true);
  check("action is 'none'", state.action === "none");
}
